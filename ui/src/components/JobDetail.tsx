      ...
      {(messages.length > 0 || canChat) && (
        <div className="flex-[2] min-h-[240px] flex flex-col border-t-4 border-ink overflow-hidden">
          ...
          <div className={`px-4 py-2 border-b-2 border-ink flex-shrink-0 flex items-center gap-3 ${isWaiting ? "bg-[#b8860b]/15" : "bg-concrete"}`}>
            ...
            {isRunning && !isWaiting && <span className="font-data text-xs text-muted uppercase">Message queued for next turn</span>}
          ...
          {messages.length > 0 && (
            <div className={`flex-1 overflow-y-auto p-4 space-y-4 min-h-0 ${isWaiting ? "bg-[#b8860b]/5" : "bg-concrete"}`}>
              {messages.map((msg) => (
          ...
          {/* Input area */}
          {canChat && (
            <div className={`flex-shrink-0 p-3 ${messages.length > 0 ? "border-t-2 border-ink" : "flex-1 flex flex-col justify-end"} ${isWaiting ? "bg-[#b8860b]/10" : "bg-concrete"}`} onDrop={onDrop} onDragOver={(e) => e.preventDefault()}>
              ...
              <form onSubmit={handleReply} className="flex gap-2 items-end">
                ...
                <textarea
                  ...
                  placeholder={isPending ? "Add to prompt… (Cmd+Enter to send)" : isWaiting ? "Reply to Claude… (Cmd+Enter to send)" : isRunning ? "Queue a message… (Cmd+Enter to send)" : "Message Claude… (Cmd+Enter to send)"}
                  onPaste={onPaste}
                  rows={2}
