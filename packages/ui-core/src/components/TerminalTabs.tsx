 ...
 */
export function TerminalTabs({ project }: { project: TerminalProject }) {
  const localSeq = useRef(0);
  const make = (): Session => { localSeq.current += 1; GLOBAL_SEQ += 1; return { id: `term-${GLOBAL_SEQ}`, label: `term ${localSeq.current}` }; };

  const [sessions, setSessions] = useState<Session[]>(() => [make()]);
  const [activeId, setActiveId] = useState<string>(() => sessions[0]?.id ?? "");

  // New project → reset to one fresh terminal (skip the initial mount).
  ...
  return (
    <div className="flex flex-col h-full max-w-[840px] mx-auto w-full bg-ink border-4 border-ink brutal-shadow-muted overflow-hidden">
      <div className="flex items-center gap-1 px-2 h-10 border-b-2 border-[#2a2722] flex-shrink-0 overflow-x-auto no-scrollbar">
        <TerminalIcon className="w-3.5 h-3.5 text-[#3bd16f] flex-shrink-0 mr-1" />
        {sessions.map((s) => (
          <div
            key={s.id}
            onClick={() => setActiveId(s.id)}
            className={`group flex items-center gap-1.5 px-2.5 h-7 font-mono text-[11px] border-2 cursor-pointer flex-shrink-0 transition-colors ${s.id === activeId ? "bg-[#2a2722] border-[#3bd16f] text-[#cfe8cf]" : "border-[#2a2722] text-[#6b8a6b] hover:text-[#cfe8cf]"}`}
          >
            {s.label}
        ...
        <button onClick={addTab} className="flex items-center justify-center w-7 h-7 border-2 border-[#2a2722] text-[#6b8a6b] hover:text-[#3bd16f] hover:border-[#3bd16f] flex-shrink-0 transition-colors" title="New terminal"><Plus className="w-3.5 h-3.5" /></button>
      ...
      <div className="flex-1 relative overflow-hidden">
        {sessions.map((s) => (
          <div key={s.id} className="absolute inset-0" style={{ display: s.id === activeId ? "block" : "none" }}>
            <TerminalPanel cwd={project.localPath} active={s.id === activeId} />
          </div>
        ))}
        {sessions.length === 0 && (
          <div className="absolute inset-0 flex items-center justify-center">
            <button onClick={addTab} className="font-mono text-xs text-[#6b8a6b] hover:text-[#3bd16f] border-2 border-[#2a2722] hover:border-[#3bd16f] px-4 py-2 flex items-center gap-2 transition-colors"><Plus className="w-4 h-4" /> New terminal</button>
          </div>
        )}
      </div>
    </div>
