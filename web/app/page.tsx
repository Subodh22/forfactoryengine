...
import { setToken, engineUrl } from "@/lib/api";
    ...
    <div className="h-screen flex flex-col bg-transparent text-ink overflow-hidden">
      ...
      <header className="flex items-center gap-4 px-3 sm:px-[22px] h-[62px] border-b-4 border-ink bg-concrete flex-shrink-0">
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
          ...
          {ghLogin ? (
            ...
            <a href={`${engineUrl()}/api/github/login`} className="font-data text-[11px] px-3 py-1.5 bg-ink text-concrete uppercase brutal-press border-2 border-ink">Login with GitHub</a>
          ) : (
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="hidden lg:flex w-[262px] flex-shrink-0 border-r-4 border-ink flex-col overflow-hidden bg-concrete">
          <div className="flex-1 overflow-hidden"><MasterFeed projectId={projectId} onSelectJob={setSelectedJob} onAddJob={(pid) => { setActiveProject(pid); setTab("chat"); }} /></div>
        ...
        {feedOpen && (
          <div className="lg:hidden fixed inset-0 z-40 flex">
            ...
            <div className="relative w-72 max-w-[82vw] bg-concrete border-r-4 border-ink flex flex-col">
              ...
              <div className="flex-1 overflow-hidden"><MasterFeed projectId={projectId} onSelectJob={(id) => { setSelectedJob(id); setFeedOpen(false); }} onAddJob={(pid) => { setActiveProject(pid); setTab("chat"); setFeedOpen(false); }} /></div>
              <div className="flex-shrink-0 border-t-4 border-ink p-3"><UsagePanel /></div>
            </div>
          </div>
        )}
